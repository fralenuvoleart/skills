...
license: MIT
compatibility: Requires Kinsta MCP server configured in .roo/mcp.json; Python 3.8+; optional: ipinfo.io access for geo-IP, Chromium for PDF export
---
...
2. **Run the initial analysis:**
   ```bash
   python3 .agents/skills/kinsta-logs/scripts/analyze_logs.py --dir "$RUN_DIR"
   ```
   *Note: This script reads `$RUN_DIR/.run_state.json` to find the logs and outputs a `context.json` file containing all the raw data tables and findings.*

3. **Run the targeted probe:**
   ```bash
   python3 .agents/skills/kinsta-logs/scripts/probe_targeted.py --dir "$RUN_DIR"
   ```
   *Note: This script reads `$RUN_DIR/.run_state.json` to find the `context.json` file, extracts the target URLs mechanically, and runs the probe.*
     ...
     "evaluated_errors": [
       {
         ...
         "line": "16",
         "severity": "🟡",
         "classification": "Scanner Probe",
         "narrative": "This fatal error occurs because automated scanners are hitting an internal WordPress file directly. No visitor impact. No action required."
       }
     ],
     "probe_cross_match": "...",
     "kb_references": "...",
...
1. **Build the final Markdown report:**
    ```bash
    python3 .agents/skills/kinsta-logs/scripts/build_report.py --dir "$RUN_DIR" --findings "$RUN_DIR/{timestamp}_analyst_findings.json"
    ```
...
3. **URL spelling verification (MANDATORY):**
   ...
   python3 .agents/skills/kinsta-logs/scripts/verify_urls.py \
     "$REPORT_PATH" \
     "$(jq -r .probe_baseline "$RUN_DIR/.run_state.json")" \
     "$(jq -r .probe_targeted "$RUN_DIR/.run_state.json")" \
...
| PDF export fails/skipped | Chromium missing or npx/network unavailable | Set `CHROMIUM_BIN` to the correct path. The Markdown report is still valid on its own regardless |
...
| [`scripts/export_pdf.sh`](scripts/export_pdf.sh) | Converts the final Markdown report to PDF via Chromium (md-to-pdf + system Chromium) | **Execute** in Step 4 |
...
PDF export (Step 4) uses Chromium (md-to-pdf + system Chromium at `/usr/bin/chromium`, A4). Override with `CHROMIUM_BIN` env var.
