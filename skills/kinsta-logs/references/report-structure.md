...
#### `### 🔴 Issues Found` / `### 🟡 Warnings` (PHP Errors)
- **Source:** Builder script (`build_report.py`) rendering the LLM's `evaluated_errors` array.
- **Format:** The LLM MUST provide a structured evaluation for each error cluster found in `context.json`. The builder script will render the data table (occurrences, timestamps, IPs) followed by the LLM's classification and narrative.
- **If empty:** The builder script will omit these sections if no errors were found.
