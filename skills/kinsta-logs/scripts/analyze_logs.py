...
def analyze_error_log(logs):
    ...
    # Instead of hardcoding severity into "critical"/"medium"/"low" buckets,
    # we output raw error clusters. The LLM will classify them in Step 2.
    error_clusters = []
    ...
    def _bucket_generic(ts_list, label, what):
    ...
    _bucket_generic(ssl_ts, "SSL/certificate error",
                     "SSL handshake failed — visitors saw security warnings. Check MyKinsta → Domains → certificate status.")
    _bucket_generic(conn_ts, "Connection refused / upstream timeout",
    ...
    return error_clusters, ts_range, ip_counter, error_entries, scanner_paths, scanner_ips
...
def generate_report(site_name, error_clusters, error_meta, access_data,
    ...
    # ══════════════════════════════════════════════════════════════
    # Severity counts are now determined by the LLM in Step 2, not here.
    ...
    L.append("")
    L.append("### 🔗 Error & 404 Recommendations")
    L.append("")
    ...
    # ══════════════════════════════════════════════════════════════
    # ISSUES — The raw error clusters are now rendered by build_report.py
    # based on the LLM's structured evaluation in analyst_findings.json.
    # We emit a marker here for build_report.py to replace.
    # ══════════════════════════════════════════════════════════════
    L.append("<!-- BUILDER:ERROR_TABLES -->")
    L.append("")
...
def main():
    ...
    # Resolve state file: prefer --dir, fall back to skill_dir for backward compat
    if args.dir:
    ...
    if err_filtered:
        error_clusters, ts_range, ip_counter, error_entries, scanner_paths, scanner_ips = \
    ...
    report = generate_report(site_name, error_clusters, error_meta, access_data,
    ...
    if error_clusters:
        llm_instructions.append(
            ...
            "You MUST evaluate each error cluster and provide a structured classification (severity, root cause, narrative) "
            "in the `evaluated_errors` array in your JSON output."
        )
    ...
    # Update state file with context path (use same resolution as above)
